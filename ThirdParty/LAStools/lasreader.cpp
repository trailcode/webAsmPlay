...
{
	printf("LASreadOpener::open\n");
  if (filter) filter->reset();
  if (transform) transform->reset();

  if (file_names || other_file_name)
  {
	  printf("if (file_names || other_file_name)\n");
    use_stdin = FALSE;
...
void LASreadOpener::set_file_nameMLT(const std::string fileName)
{
	set_file_name(fileName.c_str());
}
